...
const Messages = () => {
  ...
  return (
    <div className='messages-c'>
      {chatmsg ? (
        chatmsg.map((msg) => { var receiver = msg.participants.find(p=>p._id !==userId)
          ...
          <div className="messages-c-msg" key={msg.id}>
            ...
            <div className="message-content-msg">
              <div>
                <p className='message-sender-name'> {receiver.firstname } {receiver.lastname}</p>
                <p className='message-text-msg'>{msg.lastMessage?(msg.lastMessage):('start texting')}</p>
              </div>
